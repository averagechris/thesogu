...
fn main() {
    ...
    ssl_builder
        .set_private_key_file("/etc/letsencrypt/live/www.thesogu.com/privkey.pem", SslFiletype::PEM)
        .unwrap();
    ssl_builder.set_certificate_chain_file("/etc/letsencrypt/live/www.thesogu.com/fullchain.pem").unwrap();

    let _ = HttpServer::new(|| Application::new().resource("/", |r| r.f(index)))
        .bind("0.0.0.0:443")
        .expect("Cannot bind to 443")
        .start_ssl(ssl_builder).unwrap();
